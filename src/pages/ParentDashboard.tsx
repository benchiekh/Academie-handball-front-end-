...
import { playersAPI, paymentsAPI } from '../services/api';
...
const ParentDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'players' | 'payments' | 'profile'>('players');
  ...
  const loadData = async () => {
    try {
      const [playersData, paymentsData] = await Promise.all([
        playersAPI.findMyPlayers(),
        paymentsAPI.findMyPayments(),
      ]);
  ...
  const getPlayerPayments = (playerId: number) => {
              ...
              {selectedPlayer ? (
                <div>
                  ...
                  <div className="bg-white shadow overflow-hidden sm:rounded-md">
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {getPlayerPayments(selectedPlayer.id).map((payment) => (
                    ...
                    {getPlayerPayments(selectedPlayer.id).length === 0 && (
